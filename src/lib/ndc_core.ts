// ndc_core.ts - NDC28 (NDC26b + lazy+2 + hash refresh temprano) - FIX build
...
const HB = 16, HS = 1 << HB, HSL = 12, HM = HS - 1, WIN = 4 * 1024 * 1024, MINM = 4, MAXM = 258, MINR = 8;
...
function lzEncode28(data: Uint8Array, mode: 'A' | 'B' | 'C' = 'B') {
  ...
  const runVals: number[] = []; const runLens: number[] = [];
  const lits: number[] = []; const lits4: number[][] = [[], [], [], []]; const lits16: number[][] = Array.from({ length: 16 }, () => []);
  ...
  const worth = (m: { l: number, d: number } | null) => { if (!m) return null; if (mode === 'C') return m; const b = distBucket(m.d); const need = mode === 'A' ? 4 + (b >> 2) : (b < 8 ? 4 : 4 + Math.max(0, (b - 6) >> 2)); return m.l >= need ? m : null };
  let buf: number[] = [];
  ...
  for (let i = 0; i < data.length;) {
    ...
    if (m2 && m1 && m2.l > m1.l + 2) { buf.push(data[i]); i++; if (buf.length === 255) flush(i); m1 = m2; upd(i) }
    if (m1 && (!m2 || m1.l >= m2.l)) { flush(i); types.push(1); matchLens.push(m1.l - MINM); const b = distBucket(m1.d); distBuckets.push(b); distExtras.push(m1.d - (1 << b)); distExtraBits.push(b); if (m1.l > 6) upd(i + 1); if (m1.l > 12) upd(i + (m1.l >> 1)); upd(i + m1.l - 1); i += m1.l } else { buf.push(data[i]); i++; if (buf.length === 255) flush(i) }
  }
  flush(data.length);
...
export async function compressNDC_AB(data: Uint8Array, log: (x: string) => void = () => { }): Promise<{ file: Uint8Array; hash: Uint8Array }> {
  ...
  if (data.length > 768 * 1024) {
    ...
    const estimate = (tmp: ReturnType<typeof lzEncode28>) => { let eb = 0; for (let i = 0; i < tmp.distExtraBits.length; i++)eb += tmp.distExtraBits[i]; const extras = 1 + 4 + 4 + 4 + Math.ceil(eb / 8); const rl = new Uint8Array(tmp.runLens.length * 2); const dv = new DataView(rl.buffer); tmp.runLens.forEach((x, i) => dv.setUint16(i * 2, x, false)); const base = sc(tmp.types) + sc(tmp.litLens) + sc(tmp.matchLens) + sc(tmp.distBuckets) + extras + sc(tmp.runVals) + sc(rl); const n = sc(tmp.lits); const c4 = 1 + tmp.lits4.reduce((s, a) => s + sc(a), 0); const c16 = 1 + tmp.lits16.reduce((s, a) => s + sc(a), 0); return base + Math.min(n, c4, c16) };
    let best = Infinity; for (const m of ['A', 'B', 'C'] as const) { const tmp = lzEncode28(sample, m); const score = estimate(tmp); if (score < best) { best = score; mode = m } }
  }
  const s = lzEncode28(data, mode);
  ...
  const sc = (d: Uint8Array) => { const h = huffEnc(d); const raw = 1 + 4 + d.length; const huff = h ? 1 + 4 + 256 + 4 + h.p.length : Infinity; return Math.min(raw, huff) }; const nCost = sc(s.lits); const c4Cost = 1 + s.lits4.reduce((sum, a) => sum + sc(a), 0); const c16Cost = 1 + s.lits16.reduce((sum, a) => sum + sc(a), 0); let litMode = 0; if (c4Cost < nCost && c4Cost <= c16Cost) litMode = 1; else if (c16Cost < nCost && c16Cost < c4Cost) litMode = 2;
  ...
  const f = w.finish(); log(`NDC28-${mode} LIT${litMode === 0 ? 'N' : litMode === 1 ? '4' : '16'} ${fmt(f.length)}`); return { file: f, hash };
}
